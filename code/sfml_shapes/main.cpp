  ...
  while (window.isOpen()) {
    ...
    for (auto& shape : shapes) {
      shape->move(sf::Vector2f{
          static_cast<float>(window.getSize().x) / (5.f * frame_rate),
          static_cast<float>(window.getSize().y) / (5.f * frame_rate)});
    }
...
std::vector<std::unique_ptr<sf::Shape>> create_shapes()
{
  ...
  auto rectangle =
      std::make_unique<sf::RectangleShape>(sf::Vector2f{120.f, 50.f});
